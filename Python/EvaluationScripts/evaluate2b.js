...
#####Output#####
                ...
                fs.outputFileSync(expectFile, `expect -c 'spawn sh "spawnCmd.sh"; send "4 6\\r"; interact'`); 
                ...
                if (fs.existsSync('output1.txt')) {
                  ...
                  lines.forEach((line) => {
                    if (line.indexOf('5.1') > -1 && output1Points !== 3) {
                        output1Points = 3;
                    } 
                  });
                }
...
#####Output#####
                ...
                fs.outputFileSync(expectFile, `expect -c 'spawn sh "spawnCmd.sh"; send "1 1\\r"; interact'`); 
                console.log(`Executing from ${path.basename(sourceFolder)}`)
                execSync(`sh "${expectFile}"`, {stdio: 'inherit'});
                ...
                if (fs.existsSync('output2.txt')) {
                  ...
                  const lines = output.toLowerCase().split('\n');
                  lines.forEach((line) => {
                    if (line.indexOf('1.0') > -1 && output2Points !== 3) {
                        output2Points = 3;
                    } 
                  });
                }
...
COMMENTS:
        ...
        Compilation Succ/Err(Correct Logic): 4: ${corrLogicMarks}
        Test Case 1 Passed: +3: ${output1Points}
        Test Case 2 Passed: +3: ${output2Points}
        `;
