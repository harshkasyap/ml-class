...
#####Output#####
                ...
                if (fs.existsSync(outFile)) {
                  ...
                  // Case 1 Validation
                  var regexM = /[\s\S]*?22[\s\S]*?22[\s\S]*?/;
                  var matchR = regexM.exec(output);
                  if (matchR !== null) {
                    out1Marks1 += 1;
                  }

                  var regexM = /[\s\S]*?7\/8[\s\S]*?/;
                  var matchR = regexM.exec(output);
                  if (matchR !== null) {
                    out1Marks1 += 1;
                  }

                  var regexM = /[\s\S]*?8\/1[\s\S]*?8\/3[\s\S]*?8\/5[\s\S]*?8\/7[\s\S]*?/;
                  var matchR = regexM.exec(output);
                  if (matchR !== null) {
                    out1Marks1 += 3;
                  }
...
#####Output#####
                ...
                if (fs.existsSync(outFile)) {
                  ...
                  // Case 1 Validation
                  // Case 1 Validation
                  var regexM = /[\s\S]*?28[\s\S]*?28[\s\S]*?/;
                  var matchR = regexM.exec(output);
                  if (matchR !== null) {
                    out1Marks2 += 1;
                  }

                  var regexM = /[\s\S]*?8\/9[\s\S]*?/;
                  var matchR = regexM.exec(output);
                  if (matchR !== null) {
                    out1Marks2 += 1;
                  }

                  var regexM = /[\s\S]*?9\/2[\s\S]*?9\/4[\s\S]*?9\/5[\s\S]*?9\/7[\s\S]*?/;
                  var matchR = regexM.exec(output);
                  if (matchR !== null) {
                    out1Marks2 += 3;
                  }
