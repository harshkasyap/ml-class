...
#####Input (Test Case 1)#####
18 25
Expected Output = 10010 11001 3 11011 10101

#####Output#####
                `); 
                fs.outputFileSync(spawnFile, `gcc ${file} > error.txt 2>&1 -o execObject -lm && ./execObject > output1.txt`);
                fs.outputFileSync(expectFile, `expect -c 'spawn sh "spawnCmd.sh"; send "18 25\\r"; interact'`); 
                ...
                if (fs.existsSync('output1.txt')) {
                  ...
                  lines.forEach((line) => {
                    if (output1Points !== 5) {
                        if (line.indexOf('10010') > -1) {
                            output1Points++;
                        }
                        if (line.indexOf('11001') > -1) {
                            output1Points++;
                        }
                        if (line.indexOf('3') > -1) {
                            output1Points++;
...
#####Input (Test Case 2)#####
1000 1023
Expected Output = 1111101000 1111111111 4 1000011100 1000000000

#####Output#####
                `); 
                fs.outputFileSync(spawnFile, `gcc ${file} > error.txt 2>&1 -o execObject -lm && ./execObject > output2.txt`);
                fs.outputFileSync(expectFile, `expect -c 'spawn sh "spawnCmd.sh"; send "1000 1023\\r"; interact'`); 
                console.log(`Executing from ${path.basename(sourceFolder)}`)
                execSync(`sh "${expectFile}"`, {stdio: 'inherit'});
                ...
                if (fs.existsSync('output2.txt')) {
                  ...
                  lines.forEach((line) => {
                    if (output2Points !== 5) {
                        if (line.indexOf('1111101000') > -1) {
                            output2Points++;
                        }
                        if (line.indexOf('1111111111') > -1) {
                            output2Points++;
                        }
                        if (line.indexOf('4') > -1) {
                            output2Points++;
                        }
                        if (line.indexOf('1000011100') > -1) {
                            output2Points++;
                        }
                        if (line.indexOf('1000000000') > -1) {
                            output2Points++;
                        }
                    } 
                  });
