...
DEV_TRAIN = "Brown_train.txt"
TRAIN = "train.pos"

DEV_WORDS = "test.words"
DEV_OUT = "output.pos"
...
def generate_vocab(min_cnt=2, train_fp=TRAIN):
    """
    Generate vocabulary
    """
    ...
    vocab.extend([line.strip() for line in open(UNK_TOKS, "r")])
...
def train_model(vocab, train_fp=TRAIN):
    """
...
def preprocessFile():
    ...
    testOrg = open(TEST_ORG, "w")
    testFile = open(TEST_WORDS, "w")
    
    noOfLines = 0
    with open(DEV_TRAIN, "r") as train:
        for line in train:
            noOfLines += 1
    train.close()

    noOfTrainingLines = ( noOfLines * 2 ) / 3
        ...
        for line in train:
            for element in line.split():
                if len(element.split("/")) == 2:
                    tok, tag = element.split("/")
                    if counter < noOfTrainingLines:
                        trainFile.write(tok + "\t" + tag + "\n")
                    else:
                        testOrg.write(tok + "\t" + tag + "\n")
                        testFile.write(tok + "\n")
            if counter < noOfTrainingLines:
                trainFile.write("\n")
            else:
...
def main():
    ...
    if not os.path.isfile(VOCAB):
        ...
        vocab = [line.strip() for line in open(VOCAB, "r")]

    if not os.path.isfile(MODEL):
        print("Training model...")
        model = train_model(vocab)
    else:
        model = [line.strip() for line in open(MODEL, "r")]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        ...
        sys.exit(1)
    main()
    os.system("python eval.py test.pos output.pos")
